...
module.exports = class CivisClient {
   ...
   * @param {integer} duration [3600] - lifespan of temporary credentials (in seconds)
   * @returns {Promise.<Object, Error>} an S3 credential
   */
  createTemporaryS3Credentials(duration) {
    duration = duration || 60 * 60 * 12; // 12 hours
    var self = this;
  ...
  /**
   * Delays until the given Query Job has succeeded
   * @param {integer} queryJobId
   * @param {integer} delayInSeconds [5
   * @returns {Promise.<Object, Error>} the query job object
   */
  ...
  _delayUntilQueryCompletion(queryInfo, delayInSeconds) {
    ...
    switch (queryInfo.state) {
      ...
      case 'running':
        ...
        return Promise.delay(delayInSeconds * 1000).bind(this).then(() => {
